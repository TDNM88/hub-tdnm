...
const App: React.FC = () => {
  ...
  const handleSendMessage = async () => {
    ...
    const userMessage = { role: 'user', content: newMessage, files: selectedFiles };

    const updatedChatbots = chatbots.map((chatbot) => {
      if (chatbot.id === selectedChatbotId) {
        const updatedMessages = [...chatbot.messages, userMessage];
        return { ...chatbot, messages: updatedMessages };
      }
      return {
        ...chatbot,
        messages: chatbot.messages.map(message => ({
          role: message.role as 'user' | 'assistant', // Ép kiểu role về đúng type
          content: message.content,
          files: message.files
        }))
      };
    });
    setChatbots(updatedChatbots as Chatbot[]); // Ép kiểu về Chatbot[]
    ...
    const updatedChatbotsWithResponse = chatbots.map((chatbot) => {
      if (chatbot.id === selectedChatbotId) {
        const updatedMessages = [...chatbot.messages, userMessage, { role: 'assistant', content: assistantResponse }];
        return { ...chatbot, messages: updatedMessages };
      }
      return chatbot;
    });

    setChatbots(updatedChatbotsWithResponse as Chatbot[]); // Ép kiểu về Chatbot[]
  };
